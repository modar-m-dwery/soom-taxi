...
class PaymentService:
    ...
    def open_for_trip(cls, trip, gateway_code=None):
        ...
        # الخصم الترويجيّ يُقرَّر هنا: أوّل مشوار أو رصيد إحالة. الزبون يدفع
        # الأجرة ناقص الخصم؛ صافي السائق كما هو، والفرق على المنصّة.
        from payments.services.promotions import PromotionService
        discount, reason = PromotionService.quote(
            trip.customer, ride.service_area, amount, currency,
        )
    ...
    def _post_ledger_for_payment(cls, payment, gateway):
        ...
        **قدرتها** لا على اسمها:
        ...
        if gateway.settles_to_platform:
            ...
            # المنصّة → السائق (الخصم الترويجيّ): الزبون دفع أقلّ نقدًا،
            # والسائق يستحقّ أجرته كاملة — الفرق دَينٌ على المنصّة له.
            ...
            if discount > ZERO:
                lines.append({
                    ...
                    "direction": LedgerDirection.DEBIT,
                    "amount": discount,
                    "entry_type": LedgerEntryType.PROMOTION,
                })
                lines.append({
                    "account": LedgerAccount.DRIVER,
                    "account_ref": driver_ref,
        ...
        # الترويج يُثبَّت بعد التحصيل لا قبله: الخصم يُستهلك، ومكافأة الإحالة
        # تُمنح إن كانت هذه أوّل رحلة محصَّلة للمدعوّ.
        from payments.services.promotions import PromotionService
        PromotionService.consume(payment)
        PromotionService.reward_referral(payment, payment.trip.ride.service_area)
