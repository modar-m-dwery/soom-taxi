...
class ReferralSerializer(serializers.Serializer):
    """رمز الدعوة ورصيد الزبون الترويجيّ."""

    referral_code = serializers.CharField(help_text="رمز الزبون — يشاركه مع أصدقائه.")
    referred_by_code = serializers.CharField(
        allow_null=True, help_text="رمز من دعاه، إن سجّل واحدًا."
    )
    credit_balance = serializers.CharField(help_text="رصيد ترويجيّ غير مستهلك.")
    currency = serializers.CharField()
    first_ride_discount_available = serializers.BooleanField(
        help_text="هل ما زال خصم أوّل مشوار متاحًا لهذا الزبون.",
    )
...
class MyReferralView(APIView):
    """
    «زبون يجلب زبونًا» — الرمز والرصيد.

    الرمز يُولَّد مع ملفّ الزبون. المدعوّ يرسله مرّةً واحدة قبل رحلته
    الأولى؛ والمكافأة للطرفين تُصرف بعد أن يُتمّ رحلته — لا قبل.
    """

    authentication_classes = [TokenAuthentication]
    permission_classes = [IsAuthenticated, requires_feature("referral")]
    ...
    def _payload(self, user):
        ...
        with _tx.atomic():
            ...
            if not profile.referral_code:
                # ملفٌّ أقدم من ميزة الإحالة: save() يولّد الرمز.
                profile.save()
        referrer = None
        if profile.referred_by_id:
            referrer = getattr(profile.referred_by, "customer_profile", None)
        return {
            "referral_code": profile.referral_code,
            "referred_by_code": referrer.referral_code if referrer else None,
            "credit_balance": str(PromotionService.credit_balance(user)),
            "currency": "SYP",
            "first_ride_discount_available": (
                not profile.first_ride_discount_used
                and not PromotionService.has_paid_before(user)
            ),
        }

    @extend_schema(
        tags=["Payments"], operation_id="get_my_referral",
        summary="رمز دعوتي ورصيدي", responses={200: ReferralSerializer},
    )
    def get(self, request):
        return Response(self._payload(request.user), status=status.HTTP_200_OK)

    @extend_schema(
        tags=["Payments"], operation_id="apply_referral_code",
        summary="تسجيل رمز دعوة صديق",
        request=ApplyReferralCodeSerializer,
        responses={200: ReferralSerializer, 400: ErrorResponse},
    )
    def post(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        try:
            PromotionService.apply_referral_code(request.user, serializer.validated_data["code"])
